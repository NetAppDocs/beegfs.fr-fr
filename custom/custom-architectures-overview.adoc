...
Les nœuds de fichiers doivent exécuter RedHat 8.4 et avoir accès au référentiel HA contenant les packages requis (stimulateur, corosync, clôture-agents-tous, agents-ressources). Par exemple, la commande suivante peut être exécutée pour activer le référentiel approprié sur RedHat 8 :
